...
static void VE4(uint8_t *dst) {    // vertical
  /* SAS/C 6.58 rejects non-constant brace-initializer expressions
   * (Error 20).  Materialise the values into a plain auto array first
   * and then broadcast each row. */
  const uint8_t* top = dst - BPS;
  uint8_t vals[4];
  int i;
