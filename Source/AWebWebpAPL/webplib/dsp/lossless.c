...
#include "/dsp/yuv.h"
#include "/dsp/dsp.h"
/* Upstream 0.2.0 has a stale `#include "../enc/histogram.h"' here;
 * none of the symbols from that header are referenced in this
 * translation unit, so we omit it to avoid pulling the encoder
 * sources into the decoder-only build for the AWeb WebP plugin. */
