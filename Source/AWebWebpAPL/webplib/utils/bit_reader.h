...
/* SAS/C 6.58 has no reliable native 64-bit integer operators, so use
 * the 16-bit VP8 boolean-reader variant on Amiga.  This is slower than
 * the 32-bit path but keeps all shifts and arithmetic inside 32 bits. */
...
static WEBP_INLINE void VP8LoadNewBytes(VP8BitReader* const br) {
  ...
  if (br->buf_ + sizeof(lbit_t) <= br->buf_end_) {
    ...
    br->buf_ += (BITS) >> 3;
#if defined(__SASC) || defined(AMIGA) || defined(_AMIGA) || defined(__BIG_ENDIAN__)
    /* SAS/C on m68k does not define the upstream __BIG_ENDIAN__ probe,
     * but the CPU is big-endian.  The 16-bit VP8 reader therefore must
     * consume the word as-is; byte-swapping here corrupts the lossy VP8
     * arithmetic-coded token stream. */
    bits = (bit_t)in_bits;
#else
#if (BITS == 32)
...
static WEBP_INLINE uint32_t VP8LReadOneBitUnsafe(VP8LBitReader* const br) {
  /* The Amiga/SAS-C build uses a simple 32-bit lossless bit reader.
   * Route the unsafe helper through the checked reader so VP8L never
   * depends on a 64-bit buffered window. */
#if defined(__SASC) || defined(AMIGA) || defined(_AMIGA)
  return VP8LReadOneBit(br);
#else
